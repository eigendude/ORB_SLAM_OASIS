...
Preintegrated::Preintegrated(const Bias& b_, const Calib& calib)
{
  ...
  mfImuFrequency = calib.mfImuFrequency;
  mfYamlNoiseGyro = calib.mfYamlNoiseGyro;
  mfYamlNoiseAcc = calib.mfYamlNoiseAcc;
  mfYamlGyroWalk = calib.mfYamlGyroWalk;
  mfYamlAccWalk = calib.mfYamlAccWalk;
  mfSampleNoiseGyro = calib.mfSampleNoiseGyro;
  mfSampleNoiseAcc = calib.mfSampleNoiseAcc;
  mfSampleGyroWalk = calib.mfSampleGyroWalk;
  mfSampleAccWalk = calib.mfSampleAccWalk;
  Initialize(b_);
}
...
Preintegrated::Preintegrated(Preintegrated* pImuPre)
  : dT(pImuPre->dT),
    ...
    avgA(pImuPre->avgA),
    avgW(pImuPre->avgW),
    mfImuFrequency(pImuPre->mfImuFrequency),
    mfYamlNoiseGyro(pImuPre->mfYamlNoiseGyro),
    mfYamlNoiseAcc(pImuPre->mfYamlNoiseAcc),
    mfYamlGyroWalk(pImuPre->mfYamlGyroWalk),
    mfYamlAccWalk(pImuPre->mfYamlAccWalk),
    mfSampleNoiseGyro(pImuPre->mfSampleNoiseGyro),
    mfSampleNoiseAcc(pImuPre->mfSampleNoiseAcc),
    mfSampleGyroWalk(pImuPre->mfSampleGyroWalk),
    mfSampleAccWalk(pImuPre->mfSampleAccWalk),
    bu(pImuPre->bu),
    db(pImuPre->db),
                ...
                const float& naw)
{
  mbIsSet = true;
  mfImuFrequency = 0.0f;
  mfYamlNoiseGyro = 0.0f;
  mfYamlNoiseAcc = 0.0f;
  mfYamlGyroWalk = 0.0f;
  mfYamlAccWalk = 0.0f;
  mfSampleNoiseGyro = ng;
  mfSampleNoiseAcc = na;
  mfSampleGyroWalk = ngw;
  mfSampleAccWalk = naw;
...
void Calib::SetNoiseMetadata(const float& freq,
                             const float& yamlNg,
                             const float& yamlNa,
                             const float& yamlNgw,
                             const float& yamlNaw)
{
  mfImuFrequency = freq;
  mfYamlNoiseGyro = yamlNg;
  mfYamlNoiseAcc = yamlNa;
  mfYamlGyroWalk = yamlNgw;
  mfYamlAccWalk = yamlNaw;
}
