...
InertialEdgeResidual GetInertialEdgeResidual(EdgeInertial* edge)
{
  InertialEdgeResidual residual;
  if (!edge)
    return residual;

  edge->computeError();
  residual.chi = edge->chi2();
  residual.dR = edge->error().head<3>().norm() * 180.0 / M_PI;
  residual.dV = edge->error().segment<3>(3).norm();
  residual.dP = edge->error().tail<3>().norm();
  return residual;
}

Eigen::Vector3d GetVertexVector3(g2o::SparseOptimizer& optimizer, const unsigned long id)
{
  g2o::HyperGraph::Vertex* vertex = optimizer.vertex(id);
  if (!vertex)
    return Eigen::Vector3d::Zero();

  if (VertexVelocity* velocity = dynamic_cast<VertexVelocity*>(vertex))
    return velocity->estimate();
  if (VertexGyroBias* gyroBias = dynamic_cast<VertexGyroBias*>(vertex))
    return gyroBias->estimate();
  if (VertexAccBias* accBias = dynamic_cast<VertexAccBias*>(vertex))
    return accBias->estimate();

  return Eigen::Vector3d::Zero();
}

std::string JoinUnsignedLongs(const std::vector<unsigned long>& values)
{
...
int CountValidMapPoints(KeyFrame* pKF)
{
  int count = 0;
...
int CountObservedMapPointLinks(KeyFrame* pKF)
{
  int count = 0;
  const vector<MapPoint*> mapPoints = pKF->GetMapPointMatches();
  for (size_t i = 0; i < mapPoints.size(); ++i)
  {
    if (mapPoints[i] && !mapPoints[i]->isBad())
      count += mapPoints[i]->Observations();
  }
  return count;
}

double MedianObs(std::vector<int> values)
{
  if (values.empty())
    return 0.0;
...
int Optimizer::PoseOptimization(Frame* pFrame)
{
  ...
  {
    NumericChecks::LogRejectedInvalidFrameState("pose optimization result", stage, pFrame->mnId,
                                                pFrame->mTimeStamp);
  };
                                       ...
                                       const bool& bFixScale)
{
  ...
  for (size_t i = 0, iend = vpKFs.size(); i < iend; i++)
  {
    ...
    if (pKF->bImu && pKF->mPrevKF)
    {
      ...
      ep->setVertex(
          1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKF->mPrevKF->mnId)));
                                ...
                                bool bRecInit)
{
  ...
  for (list<KeyFrame*>::iterator it = lpOptVisKFs.begin(), itEnd = lpOptVisKFs.end(); it != itEnd;
       it++)
  {
    KeyFrame* pKFi = *it;
    VertexPose* VP = new VertexPose(pKFi);
  ...
  int inertialEdges = 0;
  vector<InertialEdgeResidual> inertialResidualBefore(vei.size());
  for (size_t i = 0, iend = vei.size(); i < iend; i++)
  {
    if (!vei[i])
      continue;
    inertialResidualBefore[i] = GetInertialEdgeResidual(vei[i]);
    inertialChiBefore += inertialResidualBefore[i].chi;
    inertialEdges++;
  }
  ...
  vector<InertialEdgeResidual> inertialResidualAfter(vei.size());
  int worstInertialIndex = -1;
  double worstInertialDChi = -std::numeric_limits<double>::infinity();
  for (size_t i = 0, iend = vei.size(); i < iend; i++)
  {
    if (!vei[i])
      continue;
    inertialResidualAfter[i] = GetInertialEdgeResidual(vei[i]);
    inertialChiAfter += inertialResidualAfter[i].chi;
    const double dchi = inertialResidualAfter[i].chi - inertialResidualBefore[i].chi;
    if (dchi > worstInertialDChi)
    {
      worstInertialDChi = dchi;
      worstInertialIndex = i;
    }
  }
  ...
  const bool inertialWorse = inertialChiBefore > 0.0 && inertialChiAfter > 1.10 * inertialChiBefore;
  const bool visualWorse = err > 0.0f && err_end > 1.25f * err;
  const bool ibaJump =
      ba > 0.25 || dba > 0.05 || bg > 0.02 || dbg > 0.005 || dpos > 0.10 || dr > 2.0 || visualWorse;
  const bool newestInertialDChiLarge = worstInertialDChi > 50.0;
  const bool suspiciousLocalInertialBA = ibaJump || inertialWorse || newestInertialDChiLarge;
  if (suspiciousLocalInertialBA)
  {
    ...
    cout << "MI iba_jump: source=local_inertial_ba" << " since_init=" << sinceInit
         << " since_viba=" << sinceInit << " kfs=" << (vpOptimizableKFs.size() + lpOptVisKFs.size())
         << "/" << lFixedKeyFrames.size()
         << " edges=" << (vpEdgesMono.size() + vpEdgesStereo.size()) << "/" << inertialEdges
         << " chi_v=" << err << "->" << err_end << " chi_i=" << inertialChiBefore << "->"
         << inertialChiAfter << " ba=" << ba << " dba=" << dba << " bg=" << bg << " dbg=" << dbg
         << " vel=" << vel << " dpos=" << dpos << " dr=" << dr << " scale=1" << endl;

    vector<KeyFrame*> newestKFs;
    for (size_t i = 0; i < vpOptimizableKFs.size() && newestKFs.size() < 3; ++i)
      newestKFs.push_back(vpOptimizableKFs[i]);

    vector<unsigned long> newestIds;
    vector<int> newestVisualEdges;
    vector<int> newestInertialEdges;
    vector<int> newestMapPoints;
    vector<int> newestObs;
    for (size_t i = 0; i < newestKFs.size(); ++i)
    {
      KeyFrame* newestKF = newestKFs[i];
      newestIds.push_back(newestKF->mnId);
      newestVisualEdges.push_back(mVisEdges[newestKF->mnId]);
      newestMapPoints.push_back(CountValidMapPoints(newestKF));
      newestObs.push_back(CountObservedMapPointLinks(newestKF));

      int edgeCount = 0;
      for (size_t j = 0; j < vpOptimizableKFs.size(); ++j)
      {
        if (!vei[j])
          continue;
        KeyFrame* edgeKF = vpOptimizableKFs[j];
        if (edgeKF == newestKF || edgeKF->mPrevKF == newestKF)
          edgeCount++;
      }
      newestInertialEdges.push_back(edgeCount);
    }

    int recentMPs = 0;
    const unsigned long oldestNewestId = newestIds.empty() ? pKF->mnId : newestIds.back();
    for (list<MapPoint*>::iterator lit = lLocalMapPoints.begin(), lend = lLocalMapPoints.end();
         lit != lend; ++lit)
    {
      MapPoint* pMP = *lit;
      if (pMP && !pMP->isBad() && pMP->mnFirstKFid >= static_cast<long int>(oldestNewestId))
        recentMPs++;
    }

    double initTimestamp = pKF->mTimeStamp;
    const vector<KeyFrame*> allKFs = pCurrentMap->GetAllKeyFrames();
    for (size_t i = 0; i < allKFs.size(); ++i)
    {
      if (allKFs[i] && allKFs[i]->mnId == pCurrentMap->GetInitKFid())
      {
        initTimestamp = allKFs[i]->mTimeStamp;
        break;
      }
    }
    const double sinceInitSeconds = pKF->mTimeStamp - initTimestamp;

    unsigned long worstKFi = 0;
    unsigned long worstKFj = 0;
    double worstDt = 0.0;
    int worstSamples = 0;
    Eigen::Vector3d worstVelBefore = Eigen::Vector3d::Zero();
    Eigen::Vector3d worstVelAfter = Eigen::Vector3d::Zero();
    Eigen::Vector3d worstBaBefore = Eigen::Vector3d::Zero();
    Eigen::Vector3d worstBaAfter = Eigen::Vector3d::Zero();
    Eigen::Vector3d worstBgBefore = Eigen::Vector3d::Zero();
    Eigen::Vector3d worstBgAfter = Eigen::Vector3d::Zero();
    if (worstInertialIndex >= 0)
    {
      KeyFrame* edgeKF = vpOptimizableKFs[worstInertialIndex];
      KeyFrame* prevKF = edgeKF ? edgeKF->mPrevKF : static_cast<KeyFrame*>(NULL);
      worstKFi = prevKF ? prevKF->mnId : 0;
      worstKFj = edgeKF ? edgeKF->mnId : 0;
      if (edgeKF && edgeKF->mpImuPreintegrated)
      {
        worstDt = edgeKF->mpImuPreintegrated->dT;
        worstSamples = edgeKF->mpImuPreintegrated->MeasurementCount();
      }
      if (edgeKF)
      {
        const unsigned long velId = maxKFid + 3 * edgeKF->mnId + 1;
        const unsigned long bgId = maxKFid + 3 * edgeKF->mnId + 2;
        const unsigned long baId = maxKFid + 3 * edgeKF->mnId + 3;
        worstVelAfter = GetVertexVector3(optimizer, velId);
        worstBgAfter = GetVertexVector3(optimizer, bgId);
        worstBaAfter = GetVertexVector3(optimizer, baId);
        worstVelBefore = edgeKF->GetVelocity().cast<double>();
        const IMU::Bias edgeBiasBefore = edgeKF->GetImuBias();
        worstBaBefore << edgeBiasBefore.bax, edgeBiasBefore.bay, edgeBiasBefore.baz;
        worstBgBefore << edgeBiasBefore.bwx, edgeBiasBefore.bwy, edgeBiasBefore.bwz;
      }
    }

    cout << "MI iba_tail_pressure: source=local_inertial_ba" << " since_init_s=" << sinceInitSeconds
         << " newest_kfs=" << JoinUnsignedLongs(newestIds)
         << " visual_edges_newest=" << JoinInts(newestVisualEdges)
         << " inertial_edges_newest=" << JoinInts(newestInertialEdges)
         << " map_points_newest=" << JoinInts(newestMapPoints)
         << " obs_newest=" << JoinInts(newestObs) << " recent_mps=" << recentMPs
         << " total_visual_edges=" << (vpEdgesMono.size() + vpEdgesStereo.size())
         << " total_inertial_edges=" << inertialEdges << " worst_edge=" << worstKFi << ","
         << worstKFj << " worst_edge_dchi=" << worstInertialDChi << " chi_v=" << err << "->"
         << err_end << " chi_i=" << inertialChiBefore << "->" << inertialChiAfter << endl;

    vector<int> newestEdgeObs;
    int newestRecentMpEdges = 0;
    int lowObsEdges = 0;
    int highObsEdges = 0;
    int farPointEdges = 0;
    int nearPointEdges = 0;
    KeyFrame* newestKF = pKF;
    for (size_t i = 0; i < vpEdgeKFMono.size(); ++i)
    {
      if (vpEdgeKFMono[i] != newestKF || !vpMapPointEdgeMono[i])
        continue;
      MapPoint* pMP = vpMapPointEdgeMono[i];
      const int obs = pMP->Observations();
      newestEdgeObs.push_back(obs);
      if (pMP->mnFirstKFid >= static_cast<long int>(oldestNewestId))
        newestRecentMpEdges++;
      if (obs <= 2)
        lowObsEdges++;
      if (obs > 3)
        highObsEdges++;
      if (pMP->mTrackDepth > 10.0f)
        farPointEdges++;
      else
        nearPointEdges++;
    }
    for (size_t i = 0; i < vpEdgeKFStereo.size(); ++i)
    {
      if (vpEdgeKFStereo[i] != newestKF || !vpMapPointEdgeStereo[i])
        continue;
      MapPoint* pMP = vpMapPointEdgeStereo[i];
      const int obs = pMP->Observations();
      newestEdgeObs.push_back(obs);
    ...
    cout << "MI iba_visual_distribution: source=local_inertial_ba"
         << " newest_kf=" << newestKF->mnId << " newest_visual_edges=" << newestEdgeObs.size()
         << " newest_recent_mp_edges=" << newestRecentMpEdges
         << " median_obs=" << MedianObs(newestEdgeObs) << " low_obs_edges=" << lowObsEdges
         << " high_obs_edges=" << highObsEdges << " far_point_edges=" << farPointEdges
         << " near_point_edges=" << nearPointEdges << endl;

    int crossEpochEdges = 0;
    unsigned long minKFId = std::numeric_limits<unsigned long>::max();
    unsigned long maxKFId = 0;
    double oldestEdgeDt = 0.0;
    double newestEdgeDt = 0.0;
    double maxEdgeDt = 0.0;
    for (size_t i = 0; i < vpOptimizableKFs.size(); ++i)
    {
      if (!vei[i])
        continue;
      KeyFrame* edgeKF = vpOptimizableKFs[i];
      KeyFrame* prevKF = edgeKF ? edgeKF->mPrevKF : static_cast<KeyFrame*>(NULL);
      if (!edgeKF || !prevKF)
        continue;
      minKFId = std::min(minKFId, std::min(edgeKF->mnId, prevKF->mnId));
      maxKFId = std::max(maxKFId, std::max(edgeKF->mnId, prevKF->mnId));
      if (prevKF->mnId < pCurrentMap->GetInitKFid() || edgeKF->mnId < pCurrentMap->GetInitKFid())
        crossEpochEdges++;
      const double edgeDt = edgeKF->mpImuPreintegrated ? edgeKF->mpImuPreintegrated->dT : 0.0;
      if (edgeKF->mnId == minKFId || oldestEdgeDt == 0.0)
        oldestEdgeDt = edgeDt;
      if (edgeKF->mnId == maxKFId || newestEdgeDt == 0.0)
        newestEdgeDt = edgeDt;
      maxEdgeDt = std::max(maxEdgeDt, edgeDt);
    }
    if (minKFId == std::numeric_limits<unsigned long>::max())
      minKFId = 0;
    cout << "MI iba_epoch_check: source=local_inertial_ba" << " edge_count=" << inertialEdges
         << " cross_epoch_edges=" << crossEpochEdges
         << " oldest_epoch=" << pCurrentMap->GetInitKFid()
         << " newest_epoch=" << pCurrentMap->GetInitKFid()
         << " current_epoch=" << pCurrentMap->GetInitKFid() << " min_kf_id=" << minKFId
         << " max_kf_id=" << maxKFId << " oldest_edge_dt=" << oldestEdgeDt
         << " newest_edge_dt=" << newestEdgeDt << " max_edge_dt=" << maxEdgeDt << endl;

    if (worstInertialIndex >= 0 && vei[worstInertialIndex])
    {
      EdgeInertial* edge = vei[worstInertialIndex];
      EdgeGyroRW* gyroRW = vegr[worstInertialIndex];
      EdgeAccRW* accRW = vear[worstInertialIndex];
      IMU::Preintegrated* pInt = edge->mpInt;
      const Eigen::VectorXd edgeInfoDiag = edge->information().diagonal();
      const double edgeInfoRot = edgeInfoDiag.segment<3>(0).mean();
      const double edgeInfoVel = edgeInfoDiag.segment<3>(3).mean();
      const double edgeInfoPos = edgeInfoDiag.segment<3>(6).mean();
      const double edgeInfoBg = gyroRW ? gyroRW->information().diagonal().mean() : 0.0;
      const double edgeInfoBa = accRW ? accRW->information().diagonal().mean() : 0.0;

      cout << "MI iba_weights: source=local_inertial_ba"
           << " yaml_noise_gyro=" << (pInt ? pInt->mfYamlNoiseGyro : 0.0f)
           << " yaml_noise_acc=" << (pInt ? pInt->mfYamlNoiseAcc : 0.0f)
           << " yaml_walk_gyro=" << (pInt ? pInt->mfYamlGyroWalk : 0.0f)
           << " yaml_walk_acc=" << (pInt ? pInt->mfYamlAccWalk : 0.0f)
           << " sample_noise_gyro=" << (pInt ? pInt->mfSampleNoiseGyro : 0.0f)
           << " sample_noise_acc=" << (pInt ? pInt->mfSampleNoiseAcc : 0.0f)
           << " sample_walk_gyro=" << (pInt ? pInt->mfSampleGyroWalk : 0.0f)
           << " sample_walk_acc=" << (pInt ? pInt->mfSampleAccWalk : 0.0f)
           << " edge_info_diag_rot=" << edgeInfoRot << " edge_info_diag_vel=" << edgeInfoVel
           << " edge_info_diag_pos=" << edgeInfoPos << " edge_info_diag_bg=" << edgeInfoBg
           << " edge_info_diag_ba=" << edgeInfoBa << endl;

      cout << "MI iba_edge_worst: source=local_inertial_ba" << " kf_i=" << worstKFi
           << " kf_j=" << worstKFj << " dt=" << worstDt << " samples=" << worstSamples
           << " chi=" << inertialResidualBefore[worstInertialIndex].chi << "->"
           << inertialResidualAfter[worstInertialIndex].chi
           << " dR_res_deg=" << inertialResidualBefore[worstInertialIndex].dR << "->"
           << inertialResidualAfter[worstInertialIndex].dR
           << " dV_res=" << inertialResidualBefore[worstInertialIndex].dV << "->"
           << inertialResidualAfter[worstInertialIndex].dV
           << " dP_res=" << inertialResidualBefore[worstInertialIndex].dP << "->"
           << inertialResidualAfter[worstInertialIndex].dP << " vel_j=" << worstVelBefore.norm()
           << "->" << worstVelAfter.norm() << " ba_j=" << worstBaBefore.transpose() << "->"
           << worstBaAfter.transpose() << " bg_j=" << worstBgBefore.transpose() << "->"
           << worstBgAfter.transpose() << endl;
    }
  }
  if (pbStopFlag)
    optimizer.setForceStopFlag(pbStopFlag);
...
Eigen::MatrixXd Optimizer::Marginalize(const Eigen::MatrixXd& H, const int& start, const int& end)
{
  ...
  Hn.block(0, 0, a + c, a + c) = Hn.block(0, 0, a + c, a + c) - Hn.block(0, a + c, a + c, b) *
                                                                    invHb *
                                                                    Hn.block(a + c, 0, b, a + c);
...
int Optimizer::PoseInertialOptimizationLastKeyFrame(Frame* pFrame, bool bRecInit)
{
  ...
  pFrame->mpcpi = new ConstraintPoseImu(VP->estimate().Rwb, VP->estimate().twb, VV->estimate(),
...
void Optimizer::OptimizeEssentialGraph4DoF(Map* pMap,
                                           KeyFrame* pLoopKF,
                                           KeyFrame* pCurKF,
                                           const LoopClosing::KeyFrameAndPose& NonCorrectedSim3,
                                           const LoopClosing::KeyFrameAndPose& CorrectedSim3,
