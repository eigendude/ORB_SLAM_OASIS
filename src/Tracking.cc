...
void LogMonoInertialImuParamsOnce(const float freq,
                                  const float yamlNg,
                                  const float yamlNa,
                                  const float yamlNgw,
                                  const float yamlNaw,
                                  const IMU::Calib* calib)
{
  ...
  logged = true;
  const float sf = freq > 0.0f ? std::sqrt(freq) : 0.0f;
  const float expectedNg = sf > 0.0f ? yamlNg * sf : 0.0f;
  const float expectedNa = sf > 0.0f ? yamlNa * sf : 0.0f;
  const float expectedNgw = sf > 0.0f ? yamlNgw / sf : 0.0f;
  const float expectedNaw = sf > 0.0f ? yamlNaw / sf : 0.0f;

  cout << "MI imu_params: freq=" << freq << " yaml_noise_gyro=" << yamlNg
       << " yaml_noise_acc=" << yamlNa << " yaml_walk_gyro=" << yamlNgw
       << " yaml_walk_acc=" << yamlNaw << " used_noise_gyro=" << calib->mfSampleNoiseGyro
       << " used_noise_acc=" << calib->mfSampleNoiseAcc
       << " used_walk_gyro=" << calib->mfSampleGyroWalk
       << " used_walk_acc=" << calib->mfSampleAccWalk << " conversion=continuous_to_sample"
       << " gyro_noise_expected_sample=" << expectedNg
       << " acc_noise_expected_sample=" << expectedNa
       << " gyro_walk_expected_sample=" << expectedNgw
       << " acc_walk_expected_sample=" << expectedNaw << endl;
}
...
void Tracking::newParameterLoader(Settings* settings)
{
  ...
  mpImuCalib->SetNoiseMetadata(mImuFreq, Ng, Na, Ngw, Naw);
  LogMonoInertialImuParamsOnce(mImuFreq, Ng, Na, Ngw, Naw, mpImuCalib);
  ...
  else if (sCameraName == "KannalaBrandt8")
  {
    ...
    if (mSensor == System::STEREO || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
    {
      ...
      if (!b_miss_params)
      {
        ...
        std::cout << "- Camera2 Lapping: " << rightLappingBegin << ", " << rightLappingEnd
                  << std::endl;
      }
    }
...
bool Tracking::ParseORBParamFile(cv::FileStorage& fSettings)
{
  ...
  {
    std::cerr << "*ORBextractor.scaleFactor parameter doesn't exist or is not a real number*"
              << std::endl;
    b_miss_params = true;
  }
                                          ...
                                          string filename)
{
  ...
  if (mSensor == System::MONOCULAR)
  {
    if (mState == NOT_INITIALIZED || mState == NO_IMAGES_YET || (lastID - initID) < mMaxFrames)
      mCurrentFrame = Frame(mImGray, timestamp, mpIniORBextractor, mpORBVocabulary, mpCamera,
  ...
  else if (mSensor == System::IMU_MONOCULAR)
  {
    ...
    {
      mCurrentFrame = Frame(mImGray, timestamp, mpIniORBextractor, mpORBVocabulary, mpCamera,
                            mDistCoef, mbf, mThDepth, &mLastFrame, *mpImuCalib);
    }
    else
      mCurrentFrame = Frame(mImGray, timestamp, mpORBextractorLeft, mpORBVocabulary, mpCamera,
                            mDistCoef, mbf, mThDepth, &mLastFrame, *mpImuCalib);
...
void Tracking::Track()
{
  ...
  if (mpLocalMapper->mbBadImu)
  {
    ...
    {
      RecordTrackingFailure(TrackingFailureReason::BadImu, mCurrentFrame.mTimeStamp);
    }
    ...
    else if (mCurrentFrame.mTimeStamp > mLastFrame.mTimeStamp + 1.0)
    {
      ...
      if (mpAtlas->isInertial())
      {
        ...
        else
        {
          ...
          RecordTrackingFailure(TrackingFailureReason::TimestampJumpBeforeImuInitialization,
  ...
  else
  {
    ...
    if (!mbOnlyTracking)
    {
      ...
      if (!bOK)
      {
        ...
        {
          RecordTrackingFailure(TrackingFailureReason::LocalMapTrackingFailed,
                                mCurrentFrame.mTimeStamp);
        }
...
void Tracking::MonocularInitialization()
{
  ...
  else
  {
    ...
    if (mpCamera->ReconstructWithTwoViews(mInitialFrame.mvKeysUn, mCurrentFrame.mvKeysUn,
                                          mvIniMatches, Tcw, mvIniP3D, vbTriangulated))
    {
...
bool Tracking::TrackLocalMap()
{
  ...
  const auto logTrackFail = [&](const char* reason, int minInliers)
  {
    ...
    const bool shouldLog = lastReason != reason || lastState != mState ||
                           mCurrentFrame.mnId < lastFrame || mCurrentFrame.mnId - lastFrame > 30 ||
                           nearInit;
    if (!shouldLog)
      return;
        ...
        (mnMatchesInliers < 50 && !mpAtlas->isImuInitialized()))
    {
      if (mpAtlas->isImuInitialized())
      {
        ...
        mnLastLocalMapFailureMapPoints =
            mpAtlas->GetCurrentMap() ? static_cast<int>(mpAtlas->GetCurrentMap()->MapPointsInMap())
                                     : 0;
      }
...
bool Tracking::Relocalization()
{
  ...
  {
    NumericChecks::LogRejectedInvalidFrameState("tracking pose", stage, mCurrentFrame.mnId,
                                                mCurrentFrame.mTimeStamp, translation);
  };
...
void Tracking::ResetPreStableMonocularInertialInitialization(const std::string& reason,
                                                             bool bLocMap)
{
  Verbose::PrintMess("Pre-stable monocular-inertial init reseting", Verbose::VERBOSITY_NORMAL);
  if (mpViewer)
  {
    mpViewer->RequestStop();
    while (!mpViewer->isStopped())
  ...
  cout << "ORB pre-init reset: first_frame=" << previousFirstFrameId
       << " initial_frame=" << previousInitialFrameId << " current_frame=" << previousCurrentFrameId
       << " retry_first_frame=" << retryFirstFrameId << " reason=" << reason << endl;
