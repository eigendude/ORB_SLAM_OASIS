...
public:
  ...
  void Set(const Sophus::SE3<float>& sophTbc,
           ...
           const float& ngw,
           const float& naw);
  void SetNoiseMetadata(const float& freq,
                        const float& yamlNg,
                        const float& yamlNa,
                        const float& yamlNgw,
                        const float& yamlNaw);
...
public:
  ...
  float mfImuFrequency = 0.0f;
  float mfYamlNoiseGyro = 0.0f, mfYamlNoiseAcc = 0.0f;
  float mfYamlGyroWalk = 0.0f, mfYamlAccWalk = 0.0f;
  float mfSampleNoiseGyro = 0.0f, mfSampleNoiseAcc = 0.0f;
  float mfSampleGyroWalk = 0.0f, mfSampleAccWalk = 0.0f;
  bool mbIsSet;
};
...
public:
  ...
  Bias GetUpdatedBias();
  size_t MeasurementCount() const { return mvMeasurements.size(); }
